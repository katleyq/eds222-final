...
![Core Study Lakes, North Temperate Lake LTER.](images/Screenshot%202023-12-14%20at%2012.33.23%20PM.png){fig-align="center" width="463"}
...
lake_phys <- read_csv(here("data/phys_lake.csv")) %>% 
  filter(lakeid %in% c("AL", "BM", "CB", "CR", "SP", "TB", "TR")) %>%
...
```{r include=TRUE, warning=FALSE, message=FALSE, results = FALSE, fig.cap= "Fig 1: Nutrient, Water Temperature, Light, and Chlorophyll Histograms"}
#| code-fold: true
...
tab_model(mod,
          ...
          string.p = "P-value",
          title = "Fig 2: Multiple Linear Model Output",
          digits = 4)
```

Now, I want to assess the validity of my linear model by plotting the residuals:

```{r include=TRUE, warning=FALSE, message=FALSE, fig.cap= "Figure 3: Plotting residuals to assess model performance"}
#| code-fold: true
...
tab_model(log_mod,
          ...
          string.ci = "Conf. Int (95%)",
          string.p = "P-value",
          title = "Fig 4. Log-odds of Algal Bloom Presence",
          digits = 4)
```
...
```{r include=TRUE, warning=FALSE, message=FALSE, fig.cap="Figure 5. Odd Ratio of Bloom Occurrence by Depth"}
#| code-fold: true
# Depth OR Plot
...
```{r include=TRUE, warning=FALSE, message=FALSE, fig.cap="Figure 6. Odds Ratio of Bloom Occurrence by Ammonium and Water Temperature Data"}
#| code-fold: true
# Ammonium OR Plot
...
```{r include=TRUE, warning=FALSE, message=FALSE, fig.cap="Figure 7. Confusion Matrix to Assess Logistic Model Performance"}
#| code-fold: true
conf_matrix <- log_mod %>%
  augment(type.predict = "response") %>%
...
kable(depth_cm_df) %>% 
  ...
  row_spec(seq(1, nrow(depth_cm_df), by = 2), background = "lightgrey") %>% 
...
```{r include=TRUE, warning=FALSE, message=FALSE, fig.cap="Figure 8. Classical Decomposition of Chlorophyll Concentrations over past 4 decades"}
#| code-fold: true
decomp <- full_data %>% 
...
Based on the results of my multiple regression model, normalized ammmonium concentration, water temperature, and light conditions can explain 23% of the variance in normalized chlorophyll concentrations in this dataset. One percentage point change in ammonium concentration results in a 0.21 change in normalized chorophyll concentration, holding all other variables constant.. Likewise, for every one degree Celsius change in water temperature, the normalized concentration of chlorophyll increases by 0.1 when all other variables are constant. Finally, for every one percentage point change in available light, the normalized chlorophyll concentration increases by 0.1 when all other variables are constant. When the residuals were plotted against the fitted values, there was an even spread of points across the plane, meaning this model met the assumptions of a linear model. Overall, this model can be improved by looking at other factors like wind and visibility to explain more of the variance in chlorophyll.

Based on the results of my logit model, a one meter increase in depth decreased the log odds by 18 percentage points. One percentage point change in ammonium concentration results in an increase in the log odds of algal bloom occurrence by a factor of 2.13. In addition, when water temperature increased by one degree Celsius, the log odds of algal bloom occurrence slightly increased by a factor of 1.20. To assess my logit model's performance, I created a confusion matrix to calculate the amount of true positives, true negatives, false positives, and false negatives. To find the accuracy level, I added the count of true positives and true negatives and divided that by the total observations. The accuracy of my model is 92.8%. The confusion matrix shows that this model is better at predicting true negatives than true positives.

Based on the results of my decomposition analysis, there has been a slight increase over chlorophyll concentration over time across all sites from 1981-2022. This positive trend is very small though as evidence by the longer grey bar on the left of the trend panel. There is a small seasonal component to chlorophyll concentration across all sites, but most of the variation is explained by random noise in the data as evidenced by the short grey bar in the random panel.

### Conclusion

Overall, ammonium concentration, water temperature, light, and depth all explain some variation in algal biomass. These explanatory variables can also help predict the presence of absence of a bloom. Additionally, chlorophyll concentration is slightly increasing over time across all lakes. For future work, I would analyze the effect of turbulence, wind and groundwater input to see if these variables can explain more of the variance in algal growth. I would also analyze the data by site to see if some lakes are increasing more in chlorophyll than others.
